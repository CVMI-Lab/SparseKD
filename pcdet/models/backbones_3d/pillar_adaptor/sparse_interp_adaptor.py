...
class SparseInterpAdaptor(PillarAdaptorTemplate):
    def __init__(self, model_cfg, in_channel, point_cloud_range):
        ...
        for i in range(len(num_filters) - 1):
            group_list.append(
                nn.Conv2d(num_filters[i]*groups, num_filters[i+1]*groups, kernel_size, groups=groups,
                          padding=int((kernel_size-1) / 2), bias=(not use_norm)))
            if use_norm:
                group_list.append(norm_layer(num_filters[i+1]*groups, eps=1e-3, momentum=0.01))
