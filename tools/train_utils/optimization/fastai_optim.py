...
def trainable_and_exclude_params(m: nn.Module):
    "Return list of trainable params in `m`."
    res = filter(lambda p: p.requires_grad and not getattr(p, 'exclude', False), m.parameters())
    return res
...
class OptimWrapper():
    ...
    def create(cls, opt_func, lr,
        ...
        split_groups = split_bn_bias(layer_groups)
        opt = opt_func([{'params': trainable_and_exclude_params(l), 'lr': 0} for l in split_groups])
        opt = cls(opt, **kwargs)
